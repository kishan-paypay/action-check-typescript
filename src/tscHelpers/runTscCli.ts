...
*/
export async function runTscCli({ workingDir, tsconfigPath, files, tscPath }: Cfg): Promise<{ output: string, error: string }> {
  ...
  const execArgs = [
    `${path.join(workingDir, tscPath || 'node_modules/typescript/bin/tsc')}`,
    '--noEmit',
    '--noErrorTruncation',
    '--pretty',
    'false',
